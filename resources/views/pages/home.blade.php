  ...
  <script>
    ...
    $('#form_translate').on('submit', function(e) {
      ...
      $.ajax({
        ...
        error: function(xhr) {
          $('.translate_box').html('<i class="text-secondary">tidak ditemukan</i>');
          alert('Error: ' + xhr.responseText);
        }
      });
    });
    ...
    function translate(data) {
      ...
      let translate = data.translate.sort((a, b) => a['word'].length - b['word'].length);
      ...
      $.each(translate, function(index, value) {
        ...
        word = highlight_text(word, highlight) + ' <i>(' + bahasa[0].replace(' ', '') + ')</i>';
        trans = value['translate'] + ' <i>(' + bahasa[1].replace(' ', '') + ')</i>';
    ...
    function highlight_text(text, highlight) {
      ...
      if (index >= 0) {
        innerHTML = innerHTML.substring(0, index) + "<span>" + innerHTML.substring(index, index + highlight
          .length) + "</span>" + innerHTML.substring(index + highlight.length);
